...
[--inputs INPUTS] [--preferences PREFERENCES] [--descriptor DESCRIPTOR] [MATCH]

Search nix packages in flakes.
";

_help_msg="$_usage_msg

Optional arguments:
  MATCH                         match string \`MATCH' in names and descriptions
...
if [[ -z "${_DESC:-}${_FUZZ:-}" ]]; then
  echo "$_as_me: You must provide a descriptor or string to match" >&2;
  usage >&2;
  exit 1;
fi
...
_QUERY='SELECT Packages.id FROM Packages INNER JOIN Descriptions ON';
_QUERY+=' Packages.descriptionId = Descriptions.id WHERE TRUE';
...
if [[ -n "${_FUZZ}" ]]; then
  _QUERY+=" AND ( ( pname LIKE '%$_FUZZ%' ) OR ( name LIKE '%$_FUZZ%' )";
  _QUERY+=" OR ( description LIKE '%$_FUZZ%' ) )";
fi
